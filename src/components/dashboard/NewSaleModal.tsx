...
// Se vier de /numbers, podemos receber grupo/número inicial já reservados.
export default function NewSaleModal({
  ...
  useEffect(()=>{(async()=>{
    if(!user) return
    // carrega todos (admin) ou só o próprio (vendedor)
    const snap = await getDocs(collection(db,'vendors'))
    const list = snap.docs.map(d=>({id:d.id, name:(d.data() as any).name ?? d.id}))
    setVendors(list)
    if (user?.uid) setValue('vendorId', user.uid, { shouldDirty:false })
  })()},[user])
        ...
        <div>
          ...
          <Select id="region" {...register('region')}>
            ...
            {(Array.isArray(REGIONS) ? REGIONS : []).map((r: any) => {
              const lblAny = (regionLabel as any)?.(r)
              const text = typeof lblAny === 'string'
                ? lblAny
                : ((r?.name || r?.city) ? `${r?.name || r?.city} (${r?.code ?? ''})` : String(r?.code ?? ''))
              const val = r?.code ?? text
              return (
